...
# Puzzle width
result = re.search('var pwidth = ([0-9]+)', html)
puzz_width = int(result.group(1))
print "Puzzle width:", puzz_width

# Puzzle height
result = re.search('var pheight = ([0-9]+)', html)
puzz_height = int(result.group(1))
print "Puzzle height:", puzz_height

# Puzzle horizontal (left) codes
puzz_left_hints = []
for result in re.finditer('leftCodes\[[0-9]+\]=\"(.*)\";', html): # todo: better regex use, can probably do this in one line
   left_hint_list = []
...
puzz_top_hints = []
for result in re.finditer('topCodes\[[0-9]+\]=\"(.*)\";', html):
   top_hint_list = []
   for r in result.group(1).split(','):
...
print "Column hints:"
print '\n'.join(map(str, puzz_top_hints))
print ""

print "Row hints:"
print '\n'.join(map(str, puzz_left_hints))
print ""
...
# Solve it and time how long it takes
